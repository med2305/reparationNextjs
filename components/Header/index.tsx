...
import MenuComponent from "./menuData";
import Cookies from 'js-cookie';
import { decodeJwt } from 'jose';

const Header = () => {
  const token = Cookies.get('token');
  const decodedToken = token && typeof token === 'string' ? decodeJwt(token) : "";
  const role: string = decodedToken ? decodedToken.role as string : "visitor";
  console.log(role);
              ...
              <div>
                ...
                >
                  <ul className="block lg:flex lg:space-x-12">
                    <MenuComponent role={role ? role : "client"} />
                  </ul>
                </nav>
              </div>
